...
//   // Fetch booked sessions for each counselor
//   const counselorsWithBookings = await Promise.all(
//     data.data.map(async (c) => {
//       try {
//         const res = await API.get(`/booking/counselor/${c._id}`);
//         return { ...c, bookedSessions: res.data.data };
//       } catch {
//         return { ...c, bookedSessions: [] };
//       }
//     })
//   );
//   setCounselors(counselorsWithBookings);
//   setFilteredCounselors(counselorsWithBookings);
// } catch (err) {
//   console.error(err);
// } finally {
//   setLoading(false);
...
//               <p><strong>Experience:</strong> {c.experience || 0} Years</p>
...
//                 {c.bookedSessions.slice(0, 3).map((s, i) => (
//                   <div key={i} className="text-sm text-gray-600 border-b last:border-none py-1 flex justify-between items-center">
//                     <span>{new Date(s.dateTime).toLocaleString()}</span>
//                     {user.role === "client" && s.client?._id === user._id && (
//                       <button
...
//                   </div>
//                 ))}
//                 {c.bookedSessions.length > 3 && (
//                   <div className="text-xs text-gray-500 mt-1">+{c.bookedSessions.length - 3} more</div>
//                 )}
//               </div>
//             )}
//           </div>
